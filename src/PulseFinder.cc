...
void PulseFinder::Initialize()
{
  ...
  tree->Branch("pulse_start", pulse_start, "pulse_start[npulses]/D");
  tree->Branch("pulse_end", pulse_end, "pulse_end[npulses]/D");
  tree->Branch("pulse_integral", pulse_integral, "pulse_integral[npulses]/D");
  tree->Branch("pulse_prompt", pulse_prompt, "pulse_prompt[npulses]/D");
  tree->Branch("pulse_saturated", pulse_saturated, "pulse_saturated[npulses]/O");
}
...
int PulseFinder::IntegralSearch(EventData* event)
{
  ...
  for (int i=0; i<ds_wfm_nsamps; i++) {
    ...
    if ( i>2 && !in_pulse && (ds_integral[i]-ds_integral[i-1] > _pulse_start_threshold) ) {
      ...
      for (int ii=(i-1)*_down_sample_factor; ii<i*_down_sample_factor; ii++) {
    ...
    else if (in_pulse && i>2) {
      if (std::abs(ds_integral[i]-ds_integral[i-1]) < std::abs(_pulse_end_threshold) )  {
        in_pulse = false;
